...
y = dataset['class']
X = dataset.drop(['class', 'cap-diameter'], axis=1)

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, stratify=y, random_state=42)

X_train['stem-width'] = X_train['stem-width'] / 10

# KNN imputacija feature-a ring-type
df = X.copy()
df.drop(['gill-spacing', 'gill-attachment', 'cap-surface', 'ring-type'], axis=1, inplace=True)
tar_pred_map = {
...
for targets, predictors in tar_pred_map.items():
    X_train[targets] = knn_impute(X_train, targets, predictors)

# mode imputacija
for i in X_train.columns:
    if X_train[i].isna().sum() != 0:
        s = X_train[i].value_counts(normalize=True)
        missing = X_train[i].isna()
        X_train.loc[missing, i] = np.random.choice(s.index, p=s.values, size=missing.sum())
...
y_tree_train = y_train.copy()
y_tree_test = y_test.copy()

Encoder_X = LabelEncoder()
for i in X_tree_train.columns:
    if X_tree_train[i].dtype == 'object':
        X_tree_train[i] = Encoder_X.fit_transform(X_tree_train[i])
        X_tree_test[i] = Encoder_X.transform(X_tree_test[i])

Encoder_Y = LabelEncoder()
y_tree_train = Encoder_Y.fit_transform(y_tree_train)
y_tree_test = Encoder_Y.transform(y_tree_test)
scaler = StandardScaler()
'''Enkodovanje za linearne modele'''

X_lin_train = pd.get_dummies(X_train, drop_first=True)
X_lin_test = pd.get_dummies(X_test, drop_first=True)
X_lin_test = X_lin_test.reindex(columns=X_lin_train.columns, fill_value=0)

X_lin_train = scaler.fit_transform(X_lin_train)
X_lin_test = scaler.transform(X_lin_test)

y_lin_train = y_tree_train.copy()
y_lin_test = y_tree_test.copy()
